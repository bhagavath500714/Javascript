...
function loadMusic(indexNumb) {
    ...
    mainAudio.src = `../../../Extra/Music/Jesus/${allMusic[indexNumb - 1].src}.mp3`;
}
...
progressArea.addEventListener("click", (e) => {
    ...
    playMusic(); //if pause is clicked and progress clicked calling playMusic function
});

//change loop, shuffle, repeat icon onclick
const repeatBtn = wrapper.querySelector("#repeat-plist");
repeatBtn.addEventListener("click", () => {
    // first we get the innertext of the icon then we'ill change accordingly
    let getText = repeatBtn.innerText; // getting innerText of icon
    // let's do different change on different icon click using switch
    switch(getText){
...
// above we just changed the icon le's work on what to do after the song ended
mainAudio.addEventListener("ended", () => {
    // according to the icon if user has set icon to loop song then we will repeat the current song 
    ...
    switch(getText){
        case "repeat": //if this icon is repeat then simply we call the next music function so the next song will play
            nextMusic();
            break;
        case "repeat_one": //if this icon is repeat_one then we'll change the current playing song current time to 0 so the song will play from bigining
...
for (let i = 0; i < allMusic.length; i++) {
    // let's pass the soong name, artist from the array to li
    let liTag = `<li li-index="${i + 1}">
        <div class="row">
        <span>${allMusic[i].name}</span>
        <p>${allMusic[i].artist}</p>
        </div>
        <audio class="${allMusic[i].src}" src="../../../Extra/Music/Jesus/${allMusic[i].src}.mp3"></audio>
    ...
    // liAudioDuration select span tag which show audio total duration and liAudio Tag select audio tag which have audio source
    // lodeddata event used to get audio total duration without playing it
...
function playingSong(){
    for (let j =0; j < allLiTags.length; j++) {
        ...
        // let's remove playing class from all li except the last one which is clicked
        if(allLiTags[j].classList.contains("playing")) {
            allLiTags[j].classList.remove("playing");

            // let's get that audio duration value and pass to .audio-duration innertext
            let adDuration = audioTag.getAttribute("t-duration");
            audioTag.innerText = adDuration; //passing t-duration value to audio duration inner text 
        }

        // if there is an li tag which li-index is equal to musicIndex then this music is playing now we'll style it
